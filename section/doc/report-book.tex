...
images jumped around and were not at the place where the students
...
\begin{itemize}[label=$\Box$]
    ...
    \item When writing the \textit{proposal} do not use the word ``proposal''
      write the document as if it would be the final paper. We see too
...
\begin{itemize}[label=$\Box$]
  ...
  \item Urls in citations are never placed in howpublished, instead we
    ...
    need to be specific.
  \item Do not use the \verb|\url={ ]| in the text, instead use a
      citation.
...
\begin{itemize}[label=$\Box$]
  ...
  \item Do not worry about the figure placement if they are at a
    ...
    phrases as seen in the following image, but you need to use the
